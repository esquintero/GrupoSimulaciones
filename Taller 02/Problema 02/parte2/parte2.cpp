...
void LatticeBoltzmann::Collision(void){
  int ix,iy,i,n0,ixc=50,iyc=100; double rho0,Jx0,Jy0,R2=std::pow(100,2);
  for(ix=0;ix<Lx;ix++) //for each cell
    for(iy=0;iy<Ly;iy++){
      //compute the macroscopic fields on the cell
      if((ix-ixc)*(ix-ixc)+(iy-iyc)*(iy-iyc)>R2 && ix>ixc){
        Jx0=-1*Jx(ix,iy,false);
        Jy0=-1*Jy(ix,iy,false);
      }
      else{
